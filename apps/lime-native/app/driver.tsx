...
/**
 * The driver context. A live job survives everything here: minimizing collapses it to a peek,
 * and Back resolves to `minimize-live-work` rather than a route pop, so no navigation gesture
 * can make an accepted trip disappear.
 */
...
export default function Driver() {
  ...
  const registry = useMemo(
    () =>
      createRegistry([
        ...
        // The offer is not a step the driver navigated to — it arrived. It draws on its own
        // surface, over a duty sheet that is suspended rather than replaced.
  ...
  const onBack = useInteractionBack({
    ...
    context: {
      ...
      // An accepted job is not something to walk backwards out of.
  ...
  return (
    <NativeShell intent={frame.shell}>
      <NativeSceneRenderer
        ...
        registry={registry}
        data={{
          // While an offer is up, the held surface still shows the duty the driver was on.
          [PRIMARY]: {
        ...
        // A hunting driver looks around, and the canvas takes that gesture. Tapping the
        // subject map opens the world out and taps back — a real composition change driven
        // from the canvas, which is the seam a future Mapbox pin-drop will use.
        onPick={() => {
