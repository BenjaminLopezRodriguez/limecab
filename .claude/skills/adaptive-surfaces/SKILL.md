...
| `overlay` | same drawer snapped to the viewport | full-height task panel | default top snap on every bottom sheet; also an explicit overlay presentation |
...
- **Do** keep rest rungs on the ladder (peek / sheet / expanded). Overlay is the default top snap of the same drawer, not a new scene and not the overflow destination. Taller work is a `TaskScene`. The sheet does not measure content and spring.
